(function () {
    ...
    StockCreateController.$inject = ['$rootScope', '$scope', '$state', '$http','$timeout','$window',
        'AlertService', '$translate', 'TableController', 'Common', 'AlertModalService', 'Stock','ErrorHandle', 'FileService'];

    function StockCreateController($rootScope, $scope, $state, $http,$timeout,$window,
                                  AlertService, $translate, TableController, Common, AlertModalService, Stock,ErrorHandle, FileService) {
        ...
        function createStock(isClose){
            Stock.create($scope.stock).then(function(data){
                if($scope.blockModal != null) $scope.blockModal.hide();
