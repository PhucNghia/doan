...
(function () {
    'use strict';
    angular.module('erpApp').controller('CategoryController', CategoryController);

    CategoryController.$inject = ['$rootScope', '$scope', '$state', '$http','$timeout',
        'AlertService', '$translate', 'TableController', 'ComboBoxController', 'AlertModalService',
        'Category', 'ErrorHandle', '$window'];

    function CategoryController($rootScope, $scope, $state, $http,$timeout,
                                  AlertService, $translate, TableController, ComboBoxController, AlertModalService,
                                   Category, ErrorHandle, $window) {
        ...
        var tableConfig = {
            tableId: "categories",               //table Id
            model: "categories",                 //model
            defaultSort:"created",          //sap xep mac dinh theo cot nao
            sortType: "desc",                //kieu sap xep
            loadFunction: Category.searchFull,     //api load du lieu
            columns: columns,               //bao gom cac cot nao
            handleAfterReload: null,        //xu ly sau khi load du lieu
            handleAfterReloadParams: null,  //tham so cho xu ly sau khi load
            deleteCallback: null,           //delete callback
            customParams: "",               //dieu kien loc ban dau
            pager_id: "table_category_pager",   //phan trang
            page_id: "category_selectize_page", //phan trang
            page_number_id: "category_selectize_pageNum",   //phan trang
        ...
        $scope.defaultDelete = function () {
        	...
        	}else TableController.defaultDelete(tableConfig.tableId,Category.deleteRecord);
        }

        $scope.deleteOne = function(id){
            UIkit.modal.confirm($translate.instant("global.actionConfirm.delete"), function () {
                Category.deleteOne(id).then(function () {
