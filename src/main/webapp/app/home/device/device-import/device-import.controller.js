(function(){
    ...
    function DeviceImportController($rootScope,$scope, $state, $timeout, $translate,
        ...
        var deviceAreaCbb = {
            id: 'deviceAreaCbb',
            url: '/api/areas',
            originParams: 'type==0;areaCode=out=(ALL, VNA)',
            valueField: 'areaCode',
            labelField: 'name',
            searchField: 'name',
        ...
        /* import excel update */
        $scope.importExcelUpdate = function(){
            var file = $("#form-file-update")[0].files[0];
            if(!file) {
                AlertService.error("Vui lòng chọn file để tiến hành import dữ liệu");
                return;
            }
            ...
            $scope.blockUI();
            DeviceImport.importExcelUpdate(file).then(function(data){
                $scope.blockModal.hide();
                UIkit.modal("#modal_update_file", {}).hide();
        ...
        $scope.cancel = function (){
            $("#form-file").val(null);
            $("#form-file-update").val(null);
        }

        // ham xoa mac dinh
        $scope.defaultDelete = function () {
            TableController.defaultDelete(tableConfig.tableId, DeviceImport.deleteRecord);
        }

        //Delete all devices in file excel
        $scope.deleteFile = function (){
            TableController.defaultDelete(tableConfig2.tableId, FileImport.deleteRecord);
        }
